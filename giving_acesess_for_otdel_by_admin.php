...
if($_SESSION['admin']){
    if(isset($_GET['submit'])){
        $conn = OpenCon();
        $name = $_GET['branch'];
        $Check="SELECT * FROM otdels WHERE ot_name='$name' LIMIT 0,1";
        $result=$conn->query($Check);
        if($result->num_rows !== 0){
            $res=$result->fetch_object();
            $doc = $_GET['nameElement'];
            if($_GET['addDoc'] !== ''){
                $registration = "INSERT INTO otdels_rights (or_ot_id, ot_rg_id, ot_dc_id) VALUES ('$res->ot_id','1','$doc')";
                $reg=$conn->query($registration);
            }
            if($_GET['addFolder'] !== ''){
                $registration = "INSERT INTO otdels_rights (or_ot_id, ot_rg_id, ot_dc_id) VALUES ('$res->ot_id','2','$doc')";
                $reg=$conn->query($registration);
            }
            if($_GET['addTiket'] !== ''){
                $registration = "INSERT INTO otdels_rights (or_ot_id, ot_rg_id, ot_dc_id) VALUES ('$res->ot_id','3','$doc')";
                $reg=$conn->query($registration);
            }
            if($_GET['delete'] !== ''){
                $registration = "INSERT INTO otdels_rights (or_ot_id, ot_rg_id, ot_dc_id) VALUES ('$res->ot_id','4','$doc')";
                $reg=$conn->query($registration);
            }
            if($_GET['read'] !== ''){
                $registration = "INSERT INTO otdels_rights (or_ot_id, ot_rg_id, ot_dc_id) VALUES ('$res->ot_id','5','$doc')";
                $reg=$conn->query($registration);
            }
            if($_GET['redact'] !== ''){
                $registration = "INSERT INTO otdels_rights (or_ot_id, ot_rg_id, ot_dc_id) VALUES ('$res->ot_id','6','$doc')";
                $reg=$conn->query($registration);
            }
            ...
            echo "<script type='text/javascript'>alert('Такого отдела не существует!');</script>";         
        }
    } 
}
else{
...
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="assets/css/giving_acesess_for_otdel_by_admin.css">
        ...
        <div class="container">
            <div class="header__inner">
                ...
                <nav class="navigation">
                    <a class="navigation__link" href="acesess_for_otdel_output.php">Проверка доступа</a>
        ...
        <div class="formLk">
            <form method="get" action="" autocomplete="on">
                ...
                <div class="date-area">
                    ...
                    <div class="branch">
                        <label class="labelGiving" for="branch">Отдел</label>
