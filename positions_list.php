...
if($_SESSION['admin']){
    ...
    if(isset($_GET['submit'])){
        $posname = $_GET['positionname'];
        $query ="SELECT * FROM positions WHERE ps_name='$posname'";
        $getposId=$conn->query($query);
        $posId=$getposId->fetch_object();
        if($getposId->num_rows == 0){
            $query = "INSERT INTO positions (ps_name) VALUES ('$posname')";
            $reg=$conn->query($query);
            header("Location: positions_list.php");
        }
        else{
            echo "<script type='text/javascript'>alert('Должность уже существует!');</script>";
        }
    }
}
else{
                ...
                <div class="date-area">
                    <div>
                        <div class="posirionname">
                            ...
                            <input type="text" name="positionname" required="required">
                        </div>
                    </div>
                    <div>
                    ...
                    <?php
                        ...
                        while($nm = $res->fetch_object()){
                            echo '<div class="position-$count"><input type="text" value="';
                            echo $nm->ps_name;
                            echo '" name="position" required="required" readonly></div>';
                            $count = $count + 1;         
                        }
                    ?>
                    </div>
